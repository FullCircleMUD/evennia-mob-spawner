...
17. **Library logs to a dedicated file** — **colocated with Evennia's other logs but distinct from them.** Default `server/logs/mob_spawner.log` (sits alongside `server.log`, `portal.log`, etc. — same directory, separate file). Library code uses Python's standard `logging` module under the `"evennia_mob_spawner"` logger name, with a `FileHandler` writing to a path configurable via settings (key TBD). Errors (decision #14), warnings (decision #15), and lifecycle events all route here. Nothing reaches `server.log` from this library.
