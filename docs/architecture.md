...
7. **Same operator command pattern as world-builder.** Scope-aware admin commands (`ms_load all` / `ms_load shard=X` / etc.), auto-installed into `AccountCmdSet` via the library's AppConfig.ready(), `cmd:superuser()` locked, `ms_` prefix. Co-installed with `evennia-shards` the scopes narrow — a command whose effect stops at the process running it is gated to the shard it acts on, and the whole-world scope is refused; see [interoperability.md](interoperability.md) for which command carries which gate and why. Operations: `ms_load` (upsert), `ms_restart` (kick the ticker without YAML reload), `ms_stop` (graceful), `ms_delete` (clean removal), `ms_status` (read-only inspect), `ms_spawn_report` (read-only per-rule population census).
...
18. **`ms_restart` is a first-class operator command.** Kicks the ticker on an existing script without re-reading YAML; preserves state. Recovery action for stuck / stopped / stalled scripts (decision #26); works when YAML is unavailable (Reader fetch failed). Sits between `ms_status` and `ms_load` on the escalation ladder. Composition of `script.stop_when_safe()` + `script.start()` — both primitives already needed for `ms_stop` and `ms_load`.
...
26. **A script marked active but carrying no live tick is a distinct state — *stalled* — and the operator commands name it.** Evennia writes the pause marker (`db._paused_time`) only when a task exists, so a `ndb._task` lost any other way leaves `is_active` True with no marker. Everything gated on that marker is then a silent no-op: `pause()`, `unpause()`, and the boot walk (`update_scripts_after_server_start` → `_unpause_task`) — so a server restart does not recover it. Only `start()` attaches a fresh task. `_is_stalled()` detects the state; it reads `ndb._task`, which exists only in the process running the script, so it is used only by commands carrying the shard gate (decision #7). Each command answers for its own job: `ms_status` names the state, which is what makes it visible at all — the state word is the tell, not the absent `next=`; `ms_stop` reports the stall rather than claiming it paused something; `ms_restart` calls `start()` and then confirms a task actually attached, so a failed recovery is not reported as a success; `ms_load` refuses the whole scope before reading any rule-set YAML, names every stalled script, and points at `ms_restart` — a stall is a ticker problem, and `ms_load`'s job is rule content. `ms_status` carries the shard gate for this reason alone; the trade is losing the router's cluster view, taken deliberately because a status command that cannot see the state being checked for is worth less than one run per shard. `ms_spawn_report` is unaffected: it is gated the opposite way for its census, keeps reporting from the shared row fields, and is not where a stall is diagnosed. Refusal is per-scope all-or-nothing for the same reason as decision #4: a partial action reported as complete is the failure being guarded against. Both the stall sighting and a failed recovery log to `mob_spawner.log` (decision #17), because the state is otherwise invisible. [TBD — needs discussion: what causes the task to be lost. Observed on a live sharded deployment across all nine scripts at once; **cause not established**. One *unproven* candidate, offered as a lead rather than a diagnosis: Evennia's `_pause_task` sets the marker from `task.next_call_time()`, and that method returns `None` when the task exists but is not running — so a pause taken over an already-stopped task would write `_paused_time = None` and then drop the task, producing exactly this state. That is a mechanism which *could* do it; there is no evidence it is what happened. Nothing in Evennia's suite exercises `next_call_time()`, `_pause_task` or `_unpause_task`, so the round trip is untested upstream. A test reproducing the mechanism would confirm or kill the candidate — it would not establish the cause of the live incident.]
