...
routes a line into ``mob_spawner.log`` (co-located with Evennia's
...
Outside an Evennia engine (any caller where Evennia is not
bootstrapped — tests, future CLI tooling), ``ms_log`` is a silent
no-op: the import of ``evennia.utils.logger`` is lazy and an
``ImportError`` is swallowed. The library deliberately does not fall
back to stderr or a local file in that case.
"""
from datetime import datetime, timezone

_LOG_FILENAME = "mob_spawner.log"
_VALID_LEVELS = ("INFO", "WARN", "ERROR")


def ms_log(message: str, level: str = "INFO") -> None:
    """Emit one line to ``mob_spawner.log``.
