...
Returns a FoundLocation that the Loader uses as its entry point. Pure
manifest navigation: reads ``index.yaml`` files level by level, follows
the query's declared values, returns the path + kind it lands on.
Library-agnostic — knows nothing about rule content.

Ported verbatim from evennia-world-builder's Finder; same manifest
convention (definitions.yaml + per-folder index.yaml with
``entries: [{name, kind}]``).
"""
